...
def main():
    ...
    WP = [(-1.65, -3.2), (-1.65, 0.35), (0.65, 0.35), (0.67, -1.65), (0.56, -3.3)]
    ...
    while robot.step(timestep) != -1:
        ...
        marker.setSFVec3f(
            [*controllers[controller_idx].get_current_target(), BALL_DIAMETER]
        )
        ...
        X_i = np.array(
            [
                ranges * np.cos(angles) + LIDAR_ROBOT_X_OFFSET,
    ...
    save_cspace(cspace)
    goal = (-1.65, 0.35)
    path = plan_path(cspace, home_position, goal)  # Returns a path in world X-Y Positions
    display_path(cspace, path)
